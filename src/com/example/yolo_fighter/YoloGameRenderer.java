...
class Skill extends YoloObject
{
	...
	boolean isPoisoned = false,isSlowDown = false,isFrozen = true;

    public Skill(float x,float y,int sprite, boolean team)
    {
    	super(x,y);
    	this.team = team;
    	this.sprite = sprite;
    	...
    	case 43://EarthWave
    	{
    		...
    		{
				YoloGameRenderer.skillTeamAVe.add(new Skill(this.x, this.y, 41, team));
				YoloGameRenderer.skillTeamAVe.add(new Skill(this.x, this.y, 42, team));
    		}
			else
			{
				YoloGameRenderer.skillTeamBVe.add(new Skill(this.x, this.y, 41, team));
				YoloGameRenderer.skillTeamBVe.add(new Skill(this.x, this.y, 42, team));
			}
    		YoloGameRenderer.hitBoxs.add(new HitBox(this.x-x_radius/2f,this.y-y_radius/2f, x_radius, y_radius, damage, frameDuration,sprite,isLeft,team,false, id));
    		break;
    	}
    	case 102://Heal
       		xEnd = 0.875f;
...
//--------------------------------------------------------------------------------------------------------------------------
		...
		case 6:
			...
			else if(y_oponnent + 1 >y-y_radius/2f && y_oponnent < y + y_radius/2f && x_oponnent + 1  > x-x_radius/2f+.5f && x_oponnent < x+x_radius/2f )
			{
		...
		case 10:
			...
			else if(y_oponnent + 1 >y-y_radius/2f && y_oponnent < y + y_radius/2f && x_oponnent + 1  > x-x_radius/2f+1.5f && x_oponnent < x+x_radius/2f +.5f )
			{
				if(ret != YoloEngine.BARREL_ATTACK)
				{
					x_texture = xStart = 0;
		...
		case 11:
			...
			else 
			{
				...
				if(y_oponnent + 1 >y-y_radius/2f && y_oponnent < y + y_radius/2f && x_oponnent + 1  > x-x_radius/2f+.5f && x_oponnent < x+x_radius/2f )
				{
...
public class YoloGameRenderer implements Renderer {
	...
	private final float MOVE_SIZE_X = 2*YoloEngine.MAX_VALUE_PLAYER_SPEED/YoloEngine.display_x*YoloEngine.LEVEL_scale; // 200/display_x
	private final float MOVE_SIZE_Y = YoloEngine.MOVE_SIZE_Y/YoloEngine.display_y*YoloEngine.LEVEL_scale; // 50/display_y
	private final float MOVE_SIZE_X1 = 160f/YoloEngine.display_x*YoloEngine.LEVEL_scale;
	private final float MOVE_SIZE_Y1 = 160f/YoloEngine.display_y*YoloEngine.LEVEL_scale;
	private final float MOVE_BALL_SIZE_X = YoloEngine.MOVE_SIZE_Y/YoloEngine.display_x*YoloEngine.LEVEL_scale; // 50/display_x
	private final float MOVE_POS_X = 25f/YoloEngine.display_x*YoloEngine.LEVEL_scale;//(YoloEngine.MOVE_X/YoloEngine.display_x - MOVE_SIZE_X/2);// /MOVE_SIZE_X;  (125-100)/display_x
	private final float MOVE_POS_Y = 50f/YoloEngine.display_y*YoloEngine.LEVEL_scale; //(YoloEngine.display_y - YoloEngine.MOVE_Y)/YoloEngine.display_y + MOVE_SIZE_Y/2; // 25/display_y == move_y/2/display_y
...
//	private final float MOVE_POS_Y1= (25f/YoloEngine.display_y);// /MOVE_SIZE_Y1 ; 
	private final float LIVE_BAR_SIZE_X_0 = YoloEngine.LIVE_BAR_SIZE/YoloEngine.display_x*YoloEngine.LEVEL_scale;
	private float LIVE_BAR_SIZE_X_1 = LIVE_BAR_SIZE_X_0;
	private final float LIVE_BAR_SIZE_Y = 30f/YoloEngine.display_y*YoloEngine.LEVEL_scale;
    public static float half_fx,half_bx,half_fy,half_by ;
	
	private float cameraPosX,joyBallX =(YoloGame.x2-25f)/YoloEngine.display_x*YoloEngine.LEVEL_scale //(YoloGame.x2/YoloEngine.display_x - MOVE_BALL_SIZE_X/2)// /MOVE_BALL_SIZE_X, (x2-25)dis_x
			,jumpBtnX = 1-125f/YoloEngine.display_x*YoloEngine.LEVEL_scale // 1/(MOVE_BALL_SIZE_X*2)-1.5f
			,shotBtnX = jumpBtnX
			,crouchBtnX = 250f/YoloEngine.display_x*YoloEngine.LEVEL_scale //2.75f
			,skillBtnX = .5f - 50f/YoloEngine.display_x*YoloEngine.LEVEL_scale //  1/(MOVE_BALL_SIZE_X*2)/2
			,liveBarX_0 = 25f/YoloEngine.display_x*YoloEngine.LEVEL_scale //(0.5f/(1f/LIVE_BAR_SIZE_Y))*(1/LIVE_BAR_SIZE_X_0);	
			,joyBallX1
			,joyBackX1
			,XADD = 0; 

	private float cameraPosY
			,jumpBtnY = 150f/YoloEngine.display_y*YoloEngine.LEVEL_scale
			,shotBtnY = 25f/YoloEngine.display_y*YoloEngine.LEVEL_scale
			,crouchBtnY = 25f/YoloEngine.display_y*YoloEngine.LEVEL_scale
			,liveBarY = 1-55f/YoloEngine.display_y*YoloEngine.LEVEL_scale // 1f/LIVE_BAR_SIZE_Y -1.75f;// 1-(25+30)/dis_x
			,joyBallY1
			,joyBackY1
			,YADD = 0; 
...
//---------------------------------------Bullet Collision--------------------------------------------------------------
	...
	private void drawControls(GL10 gl)
	{
		...
		{
			joyBallX1 = (YoloGame.x_old+YoloGame.x2-40*YoloEngine.LEVEL_scale)/YoloEngine.display_x; // MOVE_POS_X1*2 + .5f - ((YoloGame.x_old-YoloGame.x2)/YoloEngine.display_x)/MOVE_SIZE_X1;//(x2+25-80)/dis_x
			joyBallY1 = (YoloGame.y_old+YoloGame.y2-40*YoloEngine.LEVEL_scale)/YoloEngine.display_y; // MOVE_SIZE_Y1 +.5f + YoloGame.y2/YoloEngine.display_y/MOVE_SIZE_Y1;// (y2+25+40)/dis_y
			joyBackX1 = (YoloGame.x_old-80*YoloEngine.LEVEL_scale)/YoloEngine.display_x; 
			joyBackY1 = (YoloGame.y_old-80*YoloEngine.LEVEL_scale)/YoloEngine.display_y;
	...
	private void drawButtons(GL10 gl)
	{
		...
		drawSt(gl, skillBtnX + XADD - 100f/YoloEngine.display_x*YoloEngine.LEVEL_scale, YADD, MOVE_BALL_SIZE_X*2, MOVE_SIZE_Y*2,  Skill1BtnTx , Skill1BtnTy, true);
		drawSt(gl, skillBtnX + XADD, YADD, MOVE_BALL_SIZE_X*2, MOVE_SIZE_Y*2,  Skill2BtnTx , Skill2BtnTy, true);
		drawSt(gl, skillBtnX + XADD + 100f/YoloEngine.display_x*YoloEngine.LEVEL_scale, YADD, MOVE_BALL_SIZE_X*2, MOVE_SIZE_Y*2, Skill3BtnTx , Skill3BtnTy, true);
	}
...
//-------------------------------------------------Tworzenie HitBoxï¿½w----------------------------------------------------------------------------
			...
			if(sprite == 7)
			{
				if(Ve.elementAt(i).frameCounter==2)
					...
					{
						hitBoxs.add(new HitBox(Ve.elementAt(i).x + (Ve.elementAt(i).isLeft?-1:1),Ve.elementAt(i).y,
								Ve.elementAt(i).x_radius, Ve.elementAt(i).y_radius, Ve.elementAt(i).damage, Ve.elementAt(i).frameDuration,
								Ve.elementAt(i).sprite,Ve.elementAt(i).isLeft,Ve.elementAt(i).team,false, Ve.elementAt(i).id));
						YoloEngine.mMultislayer.sendHitBox(Ve.elementAt(i).x + (Ve.elementAt(i).isLeft?-1:1),
			...
			else if(sprite == 9)
			{
				if(Ve.elementAt(i).frameCounter==2)
					...
					{
						hitBoxs.add(new HitBox(Ve.elementAt(i).x + (Ve.elementAt(i).isLeft?-1:1),
								Ve.elementAt(i).y, Ve.elementAt(i).x_radius, Ve.elementAt(i).y_radius, Ve.elementAt(i).damage, Ve.elementAt(i).frameDuration,
								Ve.elementAt(i).sprite,Ve.elementAt(i).isLeft,Ve.elementAt(i).team,false, Ve.elementAt(i).id));
						YoloEngine.mMultislayer.sendHitBox(Ve.elementAt(i).x+ (Ve.elementAt(i).isLeft?-1:1),
								Ve.elementAt(i).y, Ve.elementAt(i).x_radius, Ve.elementAt(i).y_radius, Ve.elementAt(i).damage, Ve.elementAt(i).frameDuration,
			...
			else if(sprite == 10)
			{
				if(Ve.elementAt(i).frameCounter==0)
					...
					{
						hitBoxs.add(new HitBox(Ve.elementAt(i).x,
								Ve.elementAt(i).y, Ve.elementAt(i).x_radius+1, Ve.elementAt(i).y_radius+1, Ve.elementAt(i).damage, Ve.elementAt(i).frameDuration,
								Ve.elementAt(i).sprite,Ve.elementAt(i).isLeft,Ve.elementAt(i).team,false, Ve.elementAt(i).id));
						YoloEngine.mMultislayer.sendHitBox(Ve.elementAt(i).x,
								Ve.elementAt(i).y, Ve.elementAt(i).x_radius+1, Ve.elementAt(i).y_radius+1, Ve.elementAt(i).damage, Ve.elementAt(i).frameDuration,
								Ve.elementAt(i).sprite,Ve.elementAt(i).isLeft,false, Ve.elementAt(i).id);
...
//--------------------------------------------------------------------------------------------------------------------------------------------		
	...
	private void findingSkillinOpp(int i,boolean Team)
	{
		...
		for(int j =p;j<q;j++)
		{
			if(Math.abs(YoloEngine.TeamAB[j].x-Ve.elementAt(i).x)<Ve.elementAt(i).x_radius)		
				...
				{
					Skill skill = new Skill(YoloEngine.TeamAB[j].x,YoloEngine.TeamAB[j].y,sprite-87,Team);
					if(Team == YoloEngine.TeamA)
			...
			if(Team == YoloEngine.TeamA)
			{
				if(skillTeamBVe.elementAt(j).sprite>=6 && skillTeamBVe.elementAt(j).sprite <= 12 )
					if(Math.abs(skillTeamBVe.elementAt(j).x-Ve.elementAt(i).x)<Ve.elementAt(i).x_radius)			
						...
						{
							Skill skill = new Skill(skillTeamBVe.elementAt(j).x,skillTeamBVe.elementAt(j).y,sprite-87,Ve.elementAt(i).team);
							if(Team == YoloEngine.TeamA)
								skillTeamAVe.add(skill);
							else
								skillTeamBVe.add(skill);
			...
			else
			{
				if(skillTeamAVe.elementAt(j).sprite>=6 && skillTeamAVe.elementAt(j).sprite <= 12 )
					if(Math.abs(skillTeamAVe.elementAt(j).x-Ve.elementAt(i).x)<Ve.elementAt(i).x_radius)			
						...
						{
							Skill skill = new Skill(skillTeamAVe.elementAt(j).x,skillTeamAVe.elementAt(j).y,sprite-87,Ve.elementAt(i).team);
							if(Team == YoloEngine.TeamA)
								skillTeamAVe.add(skill);
							else
								skillTeamBVe.add(skill);
			...
			if(Math.abs(YoloEngine.TeamAB[j].x-Ve.elementAt(i).x)<Ve.elementAt(i).x_radius)			
				if(Math.abs(YoloEngine.TeamAB[j].y-Ve.elementAt(i).y)<Ve.elementAt(i).y_radius) // rozrï¿½nianie
				{
					if(sprite==109)
					{
						...
						{
							Skill skill = new Skill(YoloEngine.TeamAB[j].x,YoloEngine.TeamAB[j].y,sprite-87,Ve.elementAt(i).team);
							if(Team == YoloEngine.TeamA)
								skillTeamAVe.add(skill);
							else
								skillTeamBVe.add(skill);
					...
					{
						if(sprite == 104)
							if(Ve.elementAt(i).team == YoloEngine.TeamAB[YoloEngine.MyID].playerTeam )
								YoloEngine.TeamAB[YoloEngine.MyID].PlayerLive -=20;
						Skill skill = new Skill(YoloEngine.TeamAB[j].x,YoloEngine.TeamAB[j].y,sprite-87,Ve.elementAt(i).team);
						skill.x = YoloEngine.TeamAB[j].x;
	...
	private void hitBox ()
	{
		for(int i = 0;i<hitBoxs.size();i++)
		{
			...
			if(hitBoxs.elementAt(i).sprite >5 && hitBoxs.elementAt(i).sprite <11)//jeï¿½eli AI
			{
				...
				Ve.elementAt(id).x = hitBoxs.elementAt(i).x;
	...
	private void drawPlayerMag(GL10 gl)
	{
		...
		gl.glLoadIdentity();
		gl.glPushMatrix();
		gl.glTranslatef(25f/YoloEngine.display_x*YoloEngine.LEVEL_scale+XADD, 1-90f/YoloEngine.display_y*YoloEngine.LEVEL_scale+YADD, 0f);
		gl.glScalef((YoloEngine.LIVE_BAR_SIZE/YoloEngine.display_x*YoloEngine.LEVEL_scale)*(YoloEngine.TeamAB[YoloEngine.MyID].playerMag/30f),LIVE_BAR_SIZE_Y, 1f);
		gl.glMatrixMode(GL10.GL_TEXTURE);
		gl.glColor4f(1f,1f,1f,1f);
		gl.glTranslatef(0f, 0.875f, 0f);
